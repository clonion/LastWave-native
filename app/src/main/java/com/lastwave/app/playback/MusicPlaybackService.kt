...
class MusicPlaybackService : MediaBrowserServiceCompat() {
    ...
    private fun buildNotification(state: MusicPlayerState, art: Bitmap?): Notification {
        ...
        // On Android 10 and below (One UI 2.x especially), our custom
        // RemoteViews player combined with DecoratedMediaCustomViewStyle
        // + an attached MediaSession caused SystemUI's older renderer to
        // draw its own full media chrome as a *second* layer on top of
        // our view — two overlapping players (see #21).
        //
        // Dropping the session fixed the duplication, but it also broke
        // third-party "now playing" tools like Kustom/KWGT, which read
        // Notification.EXTRA_MEDIA_SESSION off the posted notification
        // itself (not just the raw MediaSessionCompat object) to find
        // what's playing.
        //
        // Fix: on Android 10 and below, skip the custom RemoteViews and
        // the Decorated style entirely — let Android build its own plain
        // MediaStyle notification from contentTitle/contentText/largeIcon
        // + the session token. Nothing custom for SystemUI to duplicate,
        // and a standard MediaStyle notification always carries
        // EXTRA_MEDIA_SESSION, so Kustom and friends keep working.
        // Android 11+ is completely untouched below.
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
            val mediaStyle = Notification.MediaStyle()
                .setShowActionsInCompactView(0, 1, 2)
            platformSessionToken?.let(mediaStyle::setMediaSession)
