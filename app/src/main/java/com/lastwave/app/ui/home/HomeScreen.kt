            ...
            PullToRefreshBox(
                ...
                Column(
                    ...
                    LazyColumn(
                        ...
                        item(key = "quick_tiles", contentType = "quick_tiles") {
                            QuickTileGrid(
...
/**
 * Quick-access 2x2 tile grid — new top section from the redesign concept.
 * Sits above the date-grouped history list, giving one-tap access to the
 * app's other top-level destinations without a persistent tab bar.
 */
@Composable
private fun QuickTileGrid(
    onSearch: () -> Unit,
    onDiscover: () -> Unit,
    onGenres: () -> Unit,
    onFriends: () -> Unit,
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.fillMaxWidth()) {
            QuickTile(Icons.Filled.Search, "Search", tileShape, Modifier.weight(1f), onSearch)
            QuickTile(Icons.Filled.Explore, "Discover", tileShape, Modifier.weight(1f), onDiscover)
        }
        Row(horizontalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.fillMaxWidth()) {
            QuickTile(Icons.Filled.GraphicEq, "Genres", tileShape, Modifier.weight(1f), onGenres)
            QuickTile(Icons.Filled.People, "Friends", tileShape, Modifier.weight(1f), onFriends)
        }
    }
}
