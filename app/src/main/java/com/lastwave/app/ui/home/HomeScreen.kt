...
private fun TrackRow(
    ...
    val trackRowContent: @Composable () -> Unit = {
        Row(
            modifier = Modifier
                .fillMaxWidth()
                .heightIn(min = if (isNowPlaying) 56.dp else 64.dp)
                .padding(vertical = if (isNowPlaying) 2.dp else 6.dp, horizontal = if (isNowPlaying) 2.dp else 8.dp),
            verticalAlignment = Alignment.CenterVertically,
        ) {
            ArtworkImage(
                name = track.name,
